...
export default function FormLogin() {
  const { form, errors, isSubmitting, isGoogleLoading, handleChange, validateFieldIdentifier, validateFieldPassword, handleSubmit, handleGoogleLogin } = useLogin();
  ...
  return (
    <div className="relative mx-auto mb-3 w-full max-w-md">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <InputGoogle id="identifier" name="identifier" label="Email / Username" type="text" value={form.identifier} onChange={handleChange} onBlur={() => validateFieldIdentifier(form.identifier)} error={errors.identifier} placeholder="johndoe@email.com / johndoe" showValidIcon={true} />

        {/* PASSWORD */}
        <InputGoogle id="password" name="password" label="Password" type="password" placeholder="********" value={form.password} onChange={handleChange} onBlur={() => validateFieldPassword(form.password)} error={errors.password} />

        {/* REMEMBER ME + FORGOT PASSWORD */}
        <div className="flex items-center justify-between">
          <label htmlFor="remember" className="flex cursor-pointer select-none items-center gap-2 text-sm text-slate-600">
            <input type="checkbox" id="remember" name="remember" className="h-4 w-4 cursor-pointer rounded border-slate-300 accent-[#25C95F]" />
            <span>Ingat saya</span>
          </label>
          <Link href="/login" className="text-sm text-[#25C95F] hover:underline">Lupa Password?</Link>
        </div>

        {/* LOGIN BUTTON */}
        <button type="submit" disabled={isSubmitting} className="w-full rounded-xl bg-gradient-to-r from-[#7AF3AE] to-[#25C95F] py-3 font-semibold text-white shadow-md transition hover:opacity-95 active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-70">
