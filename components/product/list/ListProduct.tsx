...
interface ListProductProps { products: ProductWithStore[]; categories?: Category[]; category: string; sort: string; }

export default function ListProduct({ products, category, sort }: ListProductProps) {
  const filteredProducts = [...products]
    .filter((product: any) => {
      if (category === "semua") return true;
      return product.category?.id === category || product.product_categories?.id === category;
    })
    .sort((a, b) => {
  ...
  return (
    <div className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-6">
      {filteredProducts.map((product) => (<ProductCard key={product.id} product={product} showStoreName={true} />))}
    </div>
  );
}