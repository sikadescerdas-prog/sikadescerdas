...
export default function StructureVillage({
  structures,
}: StructureVillageProps) {
    ...
    structures.forEach((item) => {
      ...
      if (!periodMap.has(key)) {
        periodMap.set(key, {
          ...
          label: `${item.period.startYear} - ${
            item.period.endYear ?? "Sekarang"
          }`,
  ...
  const defaultPeriodKey = useMemo(() => {
    ...
    const currentPeriod = availablePeriods.find(
      (period) => period.startYear <= currentYear
    );
  ...
  const activeStructures = useMemo(() => {
    return structures.filter((item) => {
      ...
      const itemLabel = `${item.period.startYear} - ${
        item.period.endYear ?? "Sekarang"
      }`;
      ...
      const started =
        item.period.startYear <= currentYear;

      const notEnded =
        !item.period.endYear ||
        item.period.endYear >= currentYear;

      return (
        item.period.isActive ||
        (started && notEnded)
      );
    });
  }, [
    ...
    return (
      activeStructures.find((person) => {
        ...
        const isInstitution =
          pos.includes("bpd") ||
          cat.includes("bpd") ||
          grp.includes("lembaga") ||
          cat.includes("lembaga") ||
          cat.includes("pkk") ||
          cat.includes("posyandu");
  ...
  const filterPersonsByCategory = (
    keywords: string[]
  ) => {
    const list = activeStructures.filter(
      (person) => {
        const pos = person.position.toLowerCase();
        const cat = person.category.toLowerCase();
        const grp = (person.group || "").toLowerCase();
  ...
  const kepalaDesa = findPerson([
    "kepala desa",
  ]);

  const sekretarisDesa = findPerson([
    "sekretaris desa",
    "sekdes",
  ]);

  const kaurUmum = findPerson([
    "kaur tata usaha",
    "kaur umum",
    "tata usaha",
  ]);

  const kaurKeuangan = findPerson([
    "kaur keuangan",
  ]);

  const kaurPerencanaan = findPerson([
    "kaur perencanaan",
  ]);

  const kasiList = filterPersonsByCategory([
    "kasi",
    "kepala seksi",
  ]);

  const kadusList = filterPersonsByCategory([
    "kadus",
    "kepala dusun",
  ]);
  ...
  const bpdStructures =
    activeStructures.filter((person) => {
      const pos = person.position.toLowerCase();
      const cat = person.category.toLowerCase();
      const grp = (person.group || "")
        .toLowerCase();
  ...
  const nonBpdInstitutionalStructures =
    activeStructures.filter((person) => {
      const pos = person.position.toLowerCase();
      const cat = person.category.toLowerCase();
      const grp = (person.group || "")
      ...
      const isGovernment =
        pos.includes("kepala desa") ||
        cat.includes("kepala desa") ||
        pos.includes("sekretaris") ||
        cat.includes("sekretaris") ||
        pos.includes("kaur") ||
        cat.includes("kaur") ||
        pos.includes("kasi") ||
        cat.includes("kasi") ||
        pos.includes("kadus") ||
        cat.includes("kadus") ||
        pos.includes("kepala dusun") ||
        cat.includes("kepala dusun");
  ...
  const groupedNonBpdInstitutions: {
    [key: string]: StructureItem[];
  } = {};


  nonBpdInstitutionalStructures.forEach(
    (item) => {
      const groupName =
        item.group ||
        item.category ||
      ...
      groupedNonBpdInstitutions[groupName].push(
        item
      );
    }
  );
  ...
  const sortInstitutionMembers = (
    members: StructureItem[]
  ) => {
  ...
  const sortedBpdMembers =
    sortInstitutionMembers(
      bpdStructures
    );


  const renderCard = (
    person: StructureItem | null,
          ...
          <div className="bg-slate-50 rounded-[1.25rem] border-2 border-dashed border-slate-200 opacity-70">
            ...
            <div className="p-3">
              ...
              <div className="text-center mt-2">
                ...
                <h3 className="font-medium text-slate-400 text-xs truncate">
                  {fallbackLabel}
                </h3>

                <p className="text-[10px] text-slate-300">
                  {subLabel || "Belum diisi"}
                </p>
    ...
    <div className="space-y-16 mt-28">
      ...
      <div className="flex flex-col items-center text-center gap-1">
        <h1 className="text-2xl sm:text-3xl font-black text-slate-800 tracking-tight">
          STRUKTUR DESA DANASARI
        </h1>

        <p className="text-md font-medium text-slate-500 uppercase tracking-widest">
        ...
        {availablePeriods.length > 0 && (
          <div className="inline-flex items-center gap-2 mt-2">
            ...
            <select
              ...
              className="bg-transparent text-sm font-medium text-slate-500 uppercase tracking-widest outline-none"
            >
          ...
          <div className="w-full flex justify-between px-10 gap-20 mt-2">
            ...
            <div className="flex-1 flex flex-col items-center space-y-10">
              ...
              <div className="w-full flex flex-col items-center">
                ...
                <span className="text-[10px] font-bold text-emerald-700 bg-emerald-50 px-3 py-1 rounded-full border border-emerald-100 mb-4 uppercase tracking-wider">
                  Kepala Seksi (Kasi)
                </span>
        ...
        <div className="space-y-6 pt-8 border-t border-slate-200">
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-slate-500">
              Struktur kepengurusan dan anggota BPD desa.
            </p>
      ...
      <div className="space-y-8 pt-8 border-t border-slate-200">
        ...
        <div className="text-center max-w-xl mx-auto">
          ...
          <p className="text-sm text-slate-500 mt-1">
            Struktur kepengurusan PKK, Posyandu,
            Karang Taruna, dan lembaga desa lainnya.
          </p>
        ...
        {Object.keys(groupedNonBpdInstitutions).length > 0 ? (
          ...
          <div className="flex flex-wrap justify-center gap-8">
            ...
            {Object.entries(groupedNonBpdInstitutions).map(
              ([groupName, members]) => {

                const sortedMembers =
                  sortInstitutionMembers(members);

                return (

                  <div
                    key={groupName}
                    className="bg-slate-50 rounded-3xl p-6 border border-slate-200 shadow-sm w-full sm:w-[320px]"
                  >


                    <div className="text-center mb-6">

                      <span className="inline-block px-4 py-1.5 bg-white text-teal-700 text-xs font-bold rounded-xl shadow-sm border border-teal-100 uppercase tracking-wider">
                        {groupName}
                      </span>
                      ...
                      {sortedMembers.map(
                        (person, index) => (
                          ...
                          >
                            ...
                            {index > 0 && (
                              <div className="w-0.5 h-6 bg-teal-300" />
                            )}
          ...
          <div className="text-center py-8 bg-white rounded-2xl border border-slate-100 shadow-sm max-w-md mx-auto">
            ...
            <p className="text-sm text-slate-400">
              Belum ada data lembaga atau organisasi terdaftar.
            </p>
      ...
      <div className="bg-white rounded-[1.5rem] shadow-md border border-slate-100 overflow-hidden mt-12">
        ...
        <div className="px-6 py-5 bg-gradient-to-r from-emerald-500 to-teal-600">
          ...
          <p className="text-emerald-100 text-xs mt-1">
            Semua struktur organisasi desa untuk periode yang dipilih
          </p>
          ...
          <table className="w-full text-left">
            ...
            <thead className="bg-slate-50 border-b border-slate-100">
              ...
              <tr>
                ...
                <th className="px-4 py-3 text-xs font-bold text-slate-500 uppercase hidden sm:table-cell">
                  Kategori / Lembaga
                </th>
                ...
                activeStructures.map((item) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      ...
                      <span className="inline-block px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-700 text-xs font-medium border border-emerald-100">
