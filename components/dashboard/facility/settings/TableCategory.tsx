// components/dashboard/facility/settings/TableCategory.tsx
...
interface Props {
  onAdd: () => void;
  onEdit: (data: FacilityCategory) => void;
}

export default function TableCategory({ onAdd, onEdit }: Props) {
  const { categories, loading, removeCategory } = useFacilityCategories();

  async function handleDelete(id: string) {
    const result = await Swal.fire({
      title: "Hapus kategori?",
      text: "Semua jenis fasilitas yang terkait dengan kategori ini juga akan terhapus.",
      icon: "warning",
    ...
    <div className="overflow-hidden rounded-xl border bg-white">
      ...
      <div className="flex items-center justify-between border-b px-6 py-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold">Kategori Fasilitas</h3>
        </div>
      ...
      {categories.length === 0 ? (
        <div className="p-12 text-center">
          ...
          <p className="mt-3 text-sm text-gray-500">Belum ada kategori fasilitas</p>
        </div>
      ) : (
                ...
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex justify-end gap-2">
                      <button
                        onClick={() => onEdit(item)}
                        className="hidden rounded-lg p-2 hover:bg-gray-100"
                      >
                        <Pencil size={16} />
                      </button>
