...
export default async function PopulationPage() {
  ...
  if (!village) {
    return <main className="min-h-screen flex items-center justify-center bg-gray-50"><p className="text-slate-500">Data desa belum tersedia</p></main>;
  }

  const populations = await prisma.village_populations.findMany({
    where: { village_id: village.id },
    include: { village_population_details: { include: { population_master_items: { include: { population_categories: true } } } } },
    orderBy: { year: "desc" },
  });
  ...
  const formattedPopulations = populations.map((item) => ({
    ...
    village_population_details: item.village_population_details.map((detail) => ({
      ...
      population_master_items: {
        ...
        population_categories: { name: detail.population_master_items.population_categories.name },
      },
    })),
  }));
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      <HeaderVillage title="Data Penduduk" village={{ name: village.name, logo: village.logo_url, address: { district: village.district, regency: village.regency } }} />
