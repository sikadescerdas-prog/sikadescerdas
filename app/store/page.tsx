  ...
  return (
    <main className="min-h-screen bg-slate-50">
      ...
      <section className="mx-auto w-full max-w-7xl">
        <div className="px-4 md:pl-10 md:pr-4 flex w-full flex-col gap-5 pt-6 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex justify-start md:justify-end animate-fade-slide-delay">
            <div className="inline-flex w-full rounded-full border border-emerald-100 bg-white p-1 shadow-lg shadow-emerald-100/50 backdrop-blur-md md:w-auto">
              <button
                ...
                className={`relative flex flex-1 items-center justify-center gap-2 overflow-hidden rounded-full px-8 py-3 text-sm font-semibold transition-all duration-300 md:min-w-[150px] ${
                  tab === "product"
                    ? "bg-gradient-to-r from-emerald-600 via-green-500 to-emerald-600 text-white shadow-md shadow-emerald-200"
                    : "text-slate-500 hover:bg-emerald-50 hover:text-emerald-600"
                }`}
              >
                {tab === "product" && <span className="absolute inset-0 animate-pulse bg-white/10" />}
              ...
              >
                {tab === "store" && <span className="absolute inset-0 animate-pulse bg-white/10" />}
        ...
        <div className="mt-5 pb-8 lg:pb-16 px-3 sm:px-6">
          {tab === "product" ? (
            loading ? (
              <div className="py-10 text-center text-slate-500">Loading produk...</div>
            ) : (
              <ListProduct products={filteredProducts} categories={categories} category={category} sort={sort} />
            )
          ) : (
            <ListStore />
          )}
      ...
      <style jsx>{`
        ...
        .animate-fade-up { animation: fadeUp .6s ease-out; }
      `}</style>
    </main>
  );
}